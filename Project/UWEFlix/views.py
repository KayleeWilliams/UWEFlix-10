    ...
    try:
        for film in films:
            response = requests.get(
                f'https://api.themoviedb.org/3/find/{film.imdb}?api_key=d4c4c2d25e196ead918fc7080850a0d7&language=en-US&external_source=imdb_id')
            data = response.json()
...
def film_serialisable(film):
    serialised_showings = [showing_serialisable(
        showing) for showing in film.showings.all()]
